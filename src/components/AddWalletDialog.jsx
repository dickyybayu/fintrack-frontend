...
function AddWalletDialog({ open, onClose, onAdd }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onAdd(form);
      setSaldo("0");
      setForm({ name: '', balance: '', type: 'Bank' });
      onClose();
    } catch (err) {
      alert("Gagal menambahkan wallet.");
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={handleBackgroundClick}>
      <div ref={dialogRef} className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-4">
          <div>
            <h3 className="text-lg font-semibold">Tambah Dompet Baru</h3>
            <p className="text-sm text-gray-500">Buat dompet baru untuk mengelola sumber dana Anda</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              placeholder="Contoh: BCA, Cash, Gopay, Bitcoin"
              value={form.name}
